...
LOG="${PROJECT_ROOT}/RoverTwin/Saved/Logs/DisableCollision.log"
...
echo "=== Disable Collision (single visible RoverTwin) ==="
grep "Warning: " "$LOG" | grep -E "ROVER|DISABLE|ROOM_COLLISION" | sed 's/.*Warning: //'
grep "DISABLE_COLLISION_DONE" "$LOG" >/dev/null && echo "OK — one visible RoverTwin, collision off" || exit 1
