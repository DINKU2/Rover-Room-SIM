...
SCRIPT="${SCRIPT_DIR}/setup_rover_collision.py"
LOG="${PROJECT_ROOT}/RoverTwin/Saved/Logs/RoverCollisionSetup.log"
...
echo "=== Rover Collision Setup ==="
grep "Warning: " "$LOG" | grep -E "ROOM|GHOST|ROVER|COLLISION" | sed 's/.*Warning: //'
grep "COLLISION_SETUP_DONE" "$LOG" >/dev/null && echo "OK — physics collision enabled" || exit 1
