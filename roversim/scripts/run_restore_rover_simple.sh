...
SCRIPT="${SCRIPT_DIR}/audit_rover_scene.py"
LOG="${PROJECT_ROOT}/RoverTwin/Saved/Logs/RestoreRoverSimple.log"
...
echo "=== Restore Simple Rover ==="
grep "Warning: " "$LOG" | grep -E "ROOM|PROXY|ROVER|RESTORE" | sed 's/.*Warning: //'
grep "RESTORE_SIMPLE_DONE" "$LOG" >/dev/null && echo "OK" || exit 1
